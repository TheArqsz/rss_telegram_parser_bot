        ...
        while 1:
            try:
                ...
                for rss_url in self.rss_feeds:
                    ...
                    _current_hash = hashlib.md5((_newest_entry.link + _newest_entry.title + str(_publish_time_as_datetime)).encode()).hexdigest()
                    if _current_hash == self.rss_feeds[rss_url]['hash']:
                        ...
                        try:
                            _update_time = dateparser.parse(_newest_entry.get('updated') or datetime.datetime.now().isoformat()).strftime(" %m/%d/%Y %H:%M:%S %Z ")
